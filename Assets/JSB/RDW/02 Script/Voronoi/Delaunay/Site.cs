...
namespace csDelaunay {
	...
	public class Site : ICoord {
		...
		public List<Vector2f> RegionTwin(Rectf clippingBounds, bool FirstOne)
		{
			if (edges == null || edges.Count == 0)
			{
				return new List<Vector2f>();
			...
			{
				ReorderEdges();
				region = ClipToBoundsTwin(clippingBounds,FirstOne);
				if ((new Polygon(region)).PolyWinding() == Winding.CLOCKWISE)
				{
					region.Reverse();
				}
		...
		private List<Vector2f> ClipToBoundsTwin(Rectf bounds, bool FirstOne)
		{
			List<Vector2f> points = new List<Vector2f>();
			int n = edges.Count;
			int i = 0;
			...
			for (int j = i + 1; j < n; j++)
			{
				...
				ConnectTwin(ref points, j, bounds, FirstOne);
			}
			// Close up the polygon by adding another corner point of the bounds if needed:
			ConnectTwin(ref points, i, bounds, FirstOne, true);
		...
		private void ConnectTwin1(ref List<Vector2f> points, int j, Rectf bounds, bool FirstOne, bool closingUp = false)
		{
			Vector2f rightPoint = points[points.Count - 1];
			Edge newEdge = edges[j];
			LR newOrientation = edgeOrientations[j];
		...
		private void ConnectTwin(ref List<Vector2f> points, int j, Rectf bounds, bool FirstOne, bool closingUp = false)
		{
			Vector2f rightPoint = points[points.Count - 1];
			Edge newEdge = edges[j];
			LR newOrientation = edgeOrientations[j];
				...
				if (rightPoint.x != newPoint.x && rightPoint.y != newPoint.y)
				{
					...
					if ((rightCheck & BoundsCheck.RIGHT) != 0)
					{
						...
						if ((newCheck & BoundsCheck.BOTTOM) != 0)
						{
							...
							{
								points.Add(new Vector2f(bounds.right, bounds.top));
								points.Add(new Vector2f(bounds.left, bounds.top));
								points.Add(new Vector2f(bounds.left, bounds.bottom));
						...
						else if ((newCheck & BoundsCheck.TOP) != 0)
						{
							...
							{
								points.Add(new Vector2f(bounds.right, bounds.bottom));
								points.Add(new Vector2f(bounds.left, bounds.bottom));
								points.Add(new Vector2f(bounds.left, bounds.top));
							}
						}
